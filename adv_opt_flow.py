...
class App:
    def __init__(self, video_src, make_file = False, plot_graphs = False, full_track = False, reduction=15):
        self.track_len = 15
        ...
        if self.create_file:
            ...
            form_time = '-'.join([str(a.tm_mon), str(a.tm_mday), str(a.tm_year), str(a.tm_hour), str(a.tm_min), str(a.tm_sec),])
    ...
    def plot_graph(self):
        ...
        # plt.show()
        try:
            fig.savefig('./graph_pics/plot'+str(int((int(self.frame_idx) - 1) / 10))+'.png', dpi=fig.dpi)
    ...
    def image_with_boxes(self, image, show_image=True):
        ...
        fill_array = self.create_fill_in_array(self.rect_corners_top_bottom, corner_coordinates)
    ...
    def rect_corners(self, rect_coordinates):
        ...
        for i in range(len(x_points)-1):
            for j in range(len(y_points)-1):
                point = ((x_points[j],y_points[i], x_points[j+1],y_points[i+1]))
                coords.append(point)
        #print('coords:',coords)
        return coords
    ...
    def run(self):
        ...
        while True:
            ...
            if len(self.tracks) > 0:
                ...
                cv2.polylines(vis, [np.int32(tr) for tr in self.tracks], False, (100, 255, 0))
                cv2.putText(vis, 'track count: %d' % len(self.tracks), (20, 20), cv2.FONT_HERSHEY_PLAIN, 1.0, (255, 255, 255), lineType=cv2.LINE_AA)
            ...
            if self.frame_idx % self.detect_interval == 0:
                ...
                mask[:] = 255
                # Mask is created of same size as frame_gray with all values set to 255
                for x, y in [np.int32(tr[-1]) for tr in self.tracks]:
                    #print('inside weird for idx',self.frame_idx,'track',self.tracks)
